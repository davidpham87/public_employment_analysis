## Confirmation of the quarterly data using annual data.
...
x <- eos[[1]][country.a]
x[, egr:= 100*eg/lf, by='country'] # et: General Government employment, et: labor force
...
x[, goverment_revenue:=100*yrg/gdp]
x[, nlg_to_gdp:=100*nlg/gdp]
...
x.model.lm <- na.omit(x[, cols, with=FALSE])
...
new.data.names <- new.data <-
  c('population', 'gdp_capita', 'imf_gfs_scores', 'dpi_excecrlc', 'year_until_elections')
...
x.new[, pop:=NULL]
x.new[execrlc %in% c(0, NA, 2), execrlc:=1]
...
unr.lm <- robustnessAnalysis(x.new, cols, c('goverment_revenue', 'nlg_to_gdp'), ff)
gov.rev.lm <-  robustnessAnalysis(x.new, cols, c('unr', 'nlg_to_gdp'), ff)
nlg.lm <-  robustnessAnalysis(x.new, cols, c('unr', 'goverment_revenue'), ff)

left.lm <- robustnessAnalysis(x.new, c(cols, 'execrlc'), '', ff)
